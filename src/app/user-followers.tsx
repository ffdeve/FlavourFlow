  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await profileService.getFollowersList(userId);
        setUsers(data);
      } catch (err) {
            ...
            ListEmptyComponent={
              <View style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingTop: 96 }}>
                <View style={{
                  ...
                  <Feather name={isPrivate ? "lock" : "users"} size={28} color="#FBA82E" />
                </View>
                <Text style={{ fontFamily: "PlusJakartaSans_600SemiBold", fontSize: 16, color: "#3B3328" }}>
                  {isPrivate ? "This account is private" : "No followers yet"}
                </Text>
                {!isPrivate && (
